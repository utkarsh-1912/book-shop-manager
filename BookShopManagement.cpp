...
void purchases::new_ord()
{
	...
	cin >> eta;
	stmt.str("");
	stmt << "Insert into purchases (book_id,supp_id,qty,dt_ord,eta) values ( " << book_id << "," << supp_id << "," << qty << ",curdate(),Date_add(curdate(), INTERVAL " << eta << " DAY)" << ";";
	query = stmt.str();
	q = query.c_str();
	mysql_query(conn,q);
